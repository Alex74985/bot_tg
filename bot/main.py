...
def submit(message):
    ...
    # if not tmp.channel_id:
    #     bot.send_message(message.chat.id, 'Не указаны каналы проверки (первый указывайте канал публикации)!')
    #     return None

    base.new(models.DrawNot, tmp.id, tmp.user_id, tmp.chanel_id, '-', tmp.text, tmp.file_type, tmp.file_id, tmp.winers_count, tmp.post_time, tmp.end_time, tmp.channels)
    base.delete(models.DrawProgress, user_id=(str(message.chat.id)))
    base.delete(models.State, user_id=(str(message.chat.id)))
